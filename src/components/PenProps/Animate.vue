...
function drawerUpdate(value: any) {
  ...
  m.showDuration = (value || []).reduce((a: number, b: number) => (+a) + (+b.duration), 0)
  meta2d.setValue({
    id: activePen.id,
    frames: toRaw(value || [])
  })
...
let activePen: any = reactive({})

function active(pens: Pen[]) {
  if (pens.length === 1) {
    // 使用 Object.assign 更新响应式对象而非替换引用
...
onMounted(() => {
  ...
  setTimeout(() => {
    try {
      ...
      if (activePens && activePens.length === 1) {
        active(activePens)
      }
    } catch (e) {
